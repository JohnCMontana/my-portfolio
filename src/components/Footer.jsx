import Section from './ui/Section'
import GlowButton from './ui/GlowButton'
import Contact from './ui/Contact'
import { useState } from 'react'

const Footer = () => {
  const [isContactOpen, setIsContactOpen] = useState(false)
  return (
    <Section>
      <Contact 
        isOpen={isContactOpen} 
        onClose={() => setIsContactOpen(false)} 
        email="johnclyde.montana@gmail.com" 
      />
      <div className="mx-auto max-w-2xl text-center py-12">
        <h3 className="mb-6 text-3xl md:text-4xl font-semibold font-['JetBrains_Mono'] text-white">Stay curious. Stay secure.</h3>
        <div className="mb-6 flex justify-center items-center">
          <img src="/avatars/john-avatar.png" alt="John Montaña" className="w-40 h-40 mb-4 rounded-full" />
        </div>
        <GlowButton onClick={() => setIsContactOpen(true)}>Contact me</GlowButton>

        <div className="mt-6 text-sm text-white/50">© {new Date().getFullYear()} John Montaña. All rights reserved.</div>
      </div>
    </Section>
  )
}

export default Footer
